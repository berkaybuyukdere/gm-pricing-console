...
function rcUrl({ station, rc, year, month, day, duration, hh, mm }) {
  ...
  // Calendar-day arithmetic, NOT milliseconds: a ms-added drop-off that spans a
  // fall-back DST transition lands at 23:00 of the PREVIOUS day (verified under
  // TZ=Europe/Zurich, 2026-10-25: pickup 24 Oct + 3d gave a 2-day search, and
  // pickup 25 Oct + 1d gave dropoff == pickup). The Date constructor normalises
  // the overflowing day correctly across DST. Cloud (UTC) was never affected;
  // the operator-side direct/relay mode was.
  const dr = new Date(year, month - 1, day + duration);
...
function rcParse(j, meta) {
  ...
  const rows = (j.matches || [])
    ...
    // > 0, not just finite: a null amount survives the ?? chain and Number(null)
    // is 0, which isFinite happily accepts — a phantom 0.00 CHF row would take
    // rank #1, shift every rank, and poison compPrices[0] (the band's anchor).
    // A genuinely free car does not exist on this API; 0 always means "no price".
